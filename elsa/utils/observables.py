...
class Observable:
	...
	@staticmethod
	def _coordinate(x: Union[np.ndarray, torch.Tensor], entry: int, particle_id: list=[]):
	...
	@staticmethod
	def identity(x: Union[np.ndarray, torch.Tensor], particle_id: list=[]):
		"""Simply gives the input back"""
		del particle_id #not needed
		return x

	@staticmethod
	def ratio(x: Union[np.ndarray, torch.Tensor], entry_id: list=[0,1]):
		assert len(entry_id) == 2
		return x[:, entry_id[0]] / x[:, entry_id[1]]
 
	def coord_0(self, x: Union[np.ndarray, torch.Tensor], particle_id: list=[]):
		return self._coordinate(x, 0, particle_id=particle_id)

	def coord_1(self, x: Union[np.ndarray, torch.Tensor], particle_id: list=[]):
		return self._coordinate(x, 1, particle_id=particle_id)

	def coord_2(self, x: Union[np.ndarray, torch.Tensor], particle_id: list=[]):
		return self._coordinate(x, 2, particle_id=particle_id)

	def coord_i(self, x: Union[np.ndarray, torch.Tensor], particle_id: list=[]):
		entry = particle_id[0]
	...
	@staticmethod
	def _momentum(x: Union[np.ndarray, torch.Tensor], entry: int, particle_id: list):
	...
	def energy(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		return self._momentum(x, 0, particle_id)

	def x_momentum(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		return self._momentum(x, 1, particle_id)

	def x_momentum_over_abs(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
	...
	def y_momentum(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		return self._momentum(x, 2, particle_id)

	def y_momentum_over_abs(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
	...
	def z_momentum(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		return self._momentum(x, 3, particle_id)

	@staticmethod
	def momentum_product(x: Union[np.ndarray, torch.Tensor], particle_id: list=[0,1]):
	...
	@staticmethod
	def invariant_mass_square(x: Union[np.ndarray, torch.Tensor], particle_id: list):
	...
	def invariant_mass(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		"""Invariant Mass.
	...
	def transverse_mass(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		"""
		# Arguments
			input: Array with input data
				that will be used to calculate the invariant mass from.
	...
	def transverse_momentum(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
	...
	def rapidity(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		"""
	...
	@staticmethod
	def phi(x: Union[np.ndarray, torch.Tensor], particle_id: list):
		"""Azimuthal angle phi.
	...
	def pseudo_rapidity(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		"""Psudo Rapidity.
	...
	def pseudo_rapidity_cut(self, x: Union[np.ndarray, torch.Tensor], particle_id: list, cut: float=6.0):
		"""Psudo Rapidity.
		This function gives the pseudo rapidity of n particles.
	...
	def delta_phi(self, x: Union[np.ndarray, torch.Tensor], particle_id: list):
		"""Delta Phi.
	...
	def delta_rapidity(self, x: Union[np.ndarray, torch.Tensor], particle_id: list=[0,1]):
		"""Delta Rapidity.
	...
	def delta_R(self, x: Union[np.ndarray, torch.Tensor], particle_id: list=[0,1]):
		"""Delta R.
	...
	def log_delta_R(self, x: Union[np.ndarray, torch.Tensor], particle_id: list=[0,1], cut: float=0.4):
		"""
