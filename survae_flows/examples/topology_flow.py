...
from survae.transforms import AffineCouplingBijection, ActNormBijection, Reverse, Augment
...
train = CheckerboardDataset(num_points=128*100)
test = CheckerboardDataset(num_points=128*100)
...
def net(input_dim):
  return nn.Sequential(nn.Linear(input_dim, 200), nn.ReLU(),
...
model = Flow(base_dist=StandardNormal((4,)),
             transforms=[
				Augment(StandardNormal((2,)), (2,2)),
				AffineCouplingBijection(net(2)), ActNormBijection(2), 
				#AffineCouplingBijection(net(2)), ActNormBijection(2), Reverse(2),
				#AffineCouplingBijection(net(1)), ActNormBijection(2), Reverse(2),
				#AffineCouplingBijection(net(1)), ActNormBijection(2), Reverse(2),
				#AffineCouplingBijection(net(1)), ActNormBijection(2),
             ])
